...
export default class CookieService {
	...
	async deleteAuthCookies(): Promise<void> {
		const authCookies = ['accessToken', 'refreshToken', 'meData'];
		await Promise.all(authCookies.map((authCookie) => this.deleteCookie(authCookie)));
	}

	async getMeFromCookie(): Promise<MeResponseUsersApiInterface> {
